(function (root, factory) {
                ...
                <div id="qrcode" class="qrcode ts-qrcode">
                    <div data-type="mobile" class="hidden">
                        <div style="margin-top: 10px;">
                            ...
                            <div class="open-app ts-app-link"> <a style="font-size:16px;" class="btn deeplink" target="_blank" href="#"></a></div>
                        </div>
                    </div>
    ...
    var Builder = (function (options, defaultConfig, element) {
        ...
        function fetchData() {
            var _this = this;
            axios.get(resource.config.actionUrl)
                .then(function (response) {
                    ...
                    if (err.response && err.response.status && err.response.status === 401) {
                        _this.DOM.find('#intro_msg').text(resource.config["TIMEOUT"]).show();
                    } else {
                        _this.DOM.find('#intro_msg').text(resource.config["TECHNICAL_REASON_ERROR"]).show();
                    }
                    _this.DOM.find('.ts-timeout').fadeIn(function () {
                        _this.DOM.find('#qrcode').css('opacity', '1');
                    });

                    if (_this.events && _this.events['FETCH_DATA_ERROR'] !== null) {
                        var _event = _this.events['FETCH_DATA_ERROR']
                        _event.fn.call(_event.scope, err);
                    }
                });
        }
